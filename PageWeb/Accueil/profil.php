...
<?php 
  $nom = $prenom = $Email = $contact = $pays = $ville =  "";
  $base = new mysqli('localhost','bobsykodinc_carine','zvtZwe5MsKmB2EQ','bobsykodinc_falconeyes');
  $req = "SELECT * FROM userdata WHERE Pseudo = '$username'";
  $result = $base->query($req);
  if (!$result) {
    header("Location: http://www.sykodinc.com/PageWeb/Accueil/connexion.php");
  }
  else{
    while(NULL !== ($info = $result->fetch_assoc())){
      $nom = $info["Noms"];
      $prenom = $info["Prenoms"];
      $Email = $info["Email"];
...
<main class="container">
  ...
  <center>
    <p  class="intro">
      <b>DONNEES PERSONNELLES</b> 
    </p>
    <p  class="data">
      Identifiant: <?php echo "$username" ?>
        <br>
        Nom: <?php echo "$nom" ?> 
        <br>
        Prénoms: <?php echo "$prenom" ?>
        <br>
        Email: <?php echo "$Email" ?>
        <br>
        Téléphone: <?php echo "$contact" ?>
        <br>
        Domicile: <?php echo "$ville,$pays" ?>
        <br>
    </p>
    <br><!--
    <p>
    ...
    echo "Vous vous etes connecté(e) le ".$DateAndTime;
    ?>
    </span>
  </div>
